...
### 生成式结构化界面：A2UI（何时该上，而不是手搓渲染器）

LangGraph 节点产出"下一步说什么/做什么"，但当产出本身是**一棵用户要交互式
编辑的结构化界面**（不是一段文字）——比如一份大纲的章节可重排、可编辑，或
一个多步表单需要 agent 动态决定下一个字段——纯文本/markdown 装不下这种交互，
手搓渲染器又是在重新发明"预批准组件目录 + agent 填数据"这套安全模型（agent
只能拼装白名单里的组件，不能生成任意代码）。这类场景可以考虑 **A2UI**
（Google 的声明式生成式 UI 协议，Apache 2.0）经 **CopilotKit 的 AG-UI**（社区
生态里 LangGraph 官方支持的传输层，负责事件流 + 状态同步）承载：

```
LangGraph 节点（编排）→ CopilotKit AG-UI（传输，事件流+状态同步）
  → A2UI（声明式 UI 载荷：组件+数据的 JSON 树）→ React 渲染器
```

**判断信号**：agent 需要在运行时决定一棵结构、且用户需要交互式编辑这棵结构，
才值得上这一层；单纯"生成一段话/一份只读报告"不需要——继续用 LangGraph 直出
文本或结构化 JSON 交给 hand-built 组件渲染即可，不必每个功能都套一层生成式
UI 协议。这不是新平面，是 LangGraph 编排引擎的下游延伸：LangGraph 决定"要不
要生成一棵可交互的结构"，A2UI/AG-UI 只在答案是"要"的时候接管"这棵结构怎么
传给前端、怎么被安全地渲染"。引入前建议先在自己仓库里找真实落点（是否已经有
一个"AI 生成结构、用户要交互编辑"的既有需求在手搓一套 block/composer 模式），
用真实证据落一份 ADR，而不是为了赶技术潮流预先引入。
